...
# edit settings.local.json: Minecraft host/port, profiles, feature flags
...
For 24/7 on a server see `deploy/` — systemd setup, optional trusted-LAN access
using `mindserver_host` and `mindserver_allowed_origins`, and SSH tunneling.
The dashboard has no login; LAN access grants control to anyone who can reach it.
...
word-overlap retrieval with a warning. Docker: `docker-compose.yml` uses bridge networking. See README for dashboard
binding/port mapping; `host.docker.internal` reaches the host via its configured gateway.

**Phase flags** (cognition and skill library default on; memory and social default off; see `settings.js`): `use_cognition`, `use_memory`,
...
- **The mindserver has no authentication** — localhost-bound by default, with optional trusted-LAN binding and explicit browser origins. Anyone who can reach it has full control (create/configure/destroy agents, inject chat as any player). LAN access without login is an accepted choice for this deployment; it is not intended for public internet exposure.
